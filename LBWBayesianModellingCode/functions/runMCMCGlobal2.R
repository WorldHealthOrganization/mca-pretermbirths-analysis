runMCMCGlobal2<-function (method, input.data, order = NULL, matern.cov = TRUE, 
          ...
          nthin = 1, model.file.path = NULL, model.save.file.path = "R/model.txt") 
{
  ...
  jags.data <- list(y.ci = y.ci, gett.ci = (gett.ci - startyear.c + 
                    ...
                    nregions = nregions, ncov = ncov, covar_array= covar_array,
                    newFlag=newFlag
                    )
  ...
  if (method == "splines") {
    ...
    parnames <- c("mu.ct", "beta.d", "mu.beta","tau.beta",
                  "mu.beta.global","tau.beta.global",
                  "alpha","bias1", "bias2", "bias3",
                  "sigma1", "sigma2A", "sigma2S", "sigma3",
                  "sigma.delta", "sigma.y", "sigma.beta", 
                  "Z.tk", "delta.hc","tau.delta"
                  )
    ...
    #               "bias_s", "sigma1", "sigma2A", "sigma2S", "sigma3", "sigma4", "sigma5", "sigma6", "sigma7", 
    #               "bias1", "bias2", "bias3", "tau.beta")
    if (cs.smoothing) {
  ...
  saveRDS(mod, paste0("output/", 
                      substr(model.file.path, 0, as.numeric(gregexpr(".txt", model.file.path))-1),((niter-nburnin)/nthin)*nchains, ".RDS"))
